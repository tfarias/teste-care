$(function () {
    ...
    $('select[name="id_tipo_usuario"]').change(function () {
        var id = $(this).val();
        if (id == '') {
            ...
            $.post(SITE_PATH + '/tipo_usuario/carregar-permissoes', {id: id}, function (rotas) {
    ...
    $('form[name="salvar-permissoes"]').on('submit', function (e) {
        ...
        if ($form.find('select[name="id_tipo_usuario"]').val() == '') {
